  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontFamily: "'Anton',sans-serif", fontSize: "clamp(28px,6vw,46px)", color: "#FFD700", letterSpacing: ".04em", lineHeight: 1.1 }}>
          TOURNAMENT OVER
        </div>
        <div style={{ fontSize: 13, color: "rgba(255,255,255,.55)", marginTop: 6, marginBottom: 28, fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase" }}>
        ...
        {/* Champions callout */}
        {(tipChamp || predChamp) && (
          <div style={{ marginTop: 28, background: "rgba(255,176,32,.1)", border: "1px solid rgba(255,176,32,.3)", borderRadius: 14, padding: "14px 18px" }}>
            {tipChamp && (
              <div style={{ color: "#FFD700", fontWeight: 800, fontSize: 14, marginBottom: predChamp ? 8 : 0 }}>
                🎯 Tipping King: <span style={{ color: "#fff" }}>{tipChamp.name}</span> · {tipChamp.total} pts
              </div>
            )}
            {predChamp && (
              <div style={{ color: "#b89cff", fontWeight: 800, fontSize: 14 }}>
...
function Podium({ players, currentPlayerId, scoreKey = "total" }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "center", gap: 8, height: 160 }}>
      {displayOrder.map((p) => {
        const cfg = PODIUM_COLORS[p.podiumIdx];
        const isMe = p.id === currentPlayerId;
        const heights = [148, 112, 88];
        ...
        return (
          <div key={p.id} style={{
            ...
            <div style={{
              ...
              fontWeight: 800, fontSize: p.podiumIdx === 0 ? 14 : 12,
            ...
            <div style={{ color: "rgba(255,255,255,.6)", fontSize: 11, marginBottom: 6, fontWeight: 700 }}>
